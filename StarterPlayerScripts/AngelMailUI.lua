--[[
    AngelMailUI.lua — Client-side Angel Mail interface
...
function AngelMailUI.Init()
    local MailReceived = ReplicatedStorage:WaitForChild("MailReceived", 30)
    local MailCatalog = ReplicatedStorage:WaitForChild("MailCatalog", 30)

    if not MailReceived or not MailCatalog then
        warn("[AngelMailUI] Mail remotes not found")
        return
    end

    -- Listen for incoming mail
    MailReceived.OnClientEvent:Connect(function(data)
        AngelMailUI.ShowMailNotification(data)
    end)

    -- Listen for catalog response
    MailCatalog.OnClientEvent:Connect(function(data)
        cachedCategories = data.categories or {}
        cachedPlayers = data.players or {}
        AngelMailUI.PopulatePanel()
    end)
    ...
    print("[AngelMailUI] Angel Mail UI initialized (press M to open)")
end
...
function AngelMailUI.Open()
    ...
    local MailCatalogRemote = ReplicatedStorage:FindFirstChild("MailCatalog")
    if MailCatalogRemote then
        MailCatalogRemote:FireServer()
    end

    -- Slide in
    mailPanel.Position = UDim2.new(0.5, -250, 1.2, 0)
...
function AngelMailUI.PopulatePanel()
    ...
    sendBtn.MouseButton1Click:Connect(function()
        ...
        local MailSendRemote = ReplicatedStorage:FindFirstChild("MailSend")
...
function AngelMailUI.ShowMailNotification(data)
    -- Get the main UI screenGui
    local gui = playerGui:FindFirstChild("AngelCloudUI")
    if not gui then return end

    local notif = Instance.new("Frame")
    notif.Size = UDim2.new(0, 360, 0, 70)
    notif.Position = UDim2.new(0.5, -180, 0, -80)
