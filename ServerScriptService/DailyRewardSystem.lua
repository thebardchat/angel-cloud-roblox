...
-- RemoteEvents
local DailyRewardNotify -- Server -> Client: show reward popup
local DailyRewardClaim  -- Client -> Server: player claims reward
...
local GRACE_PERIOD = ONE_DAY * 2

function DailyRewardSystem.Init()
    DailyRewardNotify = Instance.new("RemoteEvent")
    DailyRewardNotify.Name = "DailyRewardNotify"
    DailyRewardNotify.Parent = ReplicatedStorage

    DailyRewardClaim = Instance.new("RemoteEvent")
    DailyRewardClaim.Name = "DailyRewardClaim"
    DailyRewardClaim.Parent = ReplicatedStorage

    DailyRewardClaim.OnServerEvent:Connect(function(player)
        DailyRewardSystem.ClaimReward(player)
    end)

    print("[DailyRewardSystem] Daily rewards initialized")
end

function DailyRewardSystem.OnPlayerJoined(player: Player)
        ...
        task.delay(4, function()
            if player.Parent == Players then
                DailyRewardNotify:FireClient(player, {
                    ...
                    reward = reward,
                    allRewards = DailyRewardSystem.GetWeekRewards(data.dailyStreak),
                })
            end
        end)
    end
...
function DailyRewardSystem.ClaimReward(player: Player)
    local data = DataManager.GetData(player)
    if not data then return end
    ...
    -- Award motes
    MoteSystem.AwardMotes(player, reward.motes, "daily_reward")
    ...
    -- Play reward sound
    pcall(SoundManager.PlaySFXForPlayer, player, "daily_reward", 0.7)
    ...
    print("[DailyRewardSystem] " .. player.Name .. " claimed Day " .. streakDay .. " reward (" .. reward.motes .. " Motes, streak " .. data.dailyStreak .. ")")
end

function DailyRewardSystem.GetWeekRewards(currentStreak: number): { any }
    local rewards = {}
