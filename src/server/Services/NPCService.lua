--[[
    NPCService.lua — Tutorial NPC "The Keeper" and dialogue system (Knit)
...
local NPCService = Knit.CreateService({
    ...
    Client = {
        ...
        NPCInteraction = Knit.CreateSignal(),
    },
})
...
local playerDialogueState: { [number]: { seenWelcome: boolean, seenFirstMote: boolean, seenGateHint: boolean, seenLinked: boolean } } = {}
...
function NPCService:KnitInit(): ()
    -- Listen for client NPC interactions via Knit signal
    self.Client.NPCInteraction:Connect(function(player: Player, npcId: string, action: string)
        if npcId == "the_keeper" then
            self:HandleKeeperInteraction(player, action)
        end
    end)

    print("[NPCService] NPC system initialized")
end
...
function NPCService:SpawnKeeper(layerFolder: Folder, spawnPosition: Vector3): Model
    ...
    prompt.Triggered:Connect(function(player: Player)
        self:HandleKeeperInteraction(player, "talk")
    end)
...
function NPCService:HandleKeeperInteraction(player: Player, action: string): ()
    ...
    -- Play NPC talk sound
    SoundManagerService:OnNPCTalk(player)

    -- Send dialogue to client
    self.Client.NPCDialogue:Fire(player, {
        npcId = "the_keeper",
        npcName = "The Keeper",
        lines = dialogue,
    })
...
function NPCService:RemovePlayer(player: Player): ()
