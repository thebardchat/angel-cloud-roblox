--[[
    WorldService.lua — Procedural cloud world builder (Knit Service)
...
function WorldService:KnitInit()
    Layers = require(ReplicatedStorage.Config.Layers)
    print("[WorldService] Building The Cloud Climb...")
    ...
    self:BuildLayerTransitions()

    -- Build the grand cloud stairway (visible from anywhere)
    self:BuildGrandStairway()

    -- Build world boundary walls (so players can't fly into the void)
    self:BuildWorldBoundaries()

    print("[WorldService] World generation complete — all 6 layers built")
end

function WorldService:BuildLayer(layerIndex: number)
    ...
    self:CreateSpawnPlatform(folder, layerDef.spawnPosition, palette, layerIndex)

    -- 2. Main cloud platforms (navigable terrain)
    self:CreateCloudPlatforms(folder, centerY, heightMin, heightMax, palette, layerIndex)

    -- 3. Floating islands
    self:CreateFloatingIslands(folder, centerY, heightMin, heightMax, palette, layerIndex)

    -- 4. Decorative clouds (non-solid, atmosphere)
    self:CreateDecorativeClouds(folder, heightMin, heightMax, palette)

    -- 5. Light pillars / glowing accents
    self:CreateLightPillars(folder, heightMin, heightMax, palette, layerIndex)

    -- 6. Reflection Pool structure
    self:CreateReflectionPool(folder, layerDef, palette)

    -- 7. Layer-specific features
    if layerIndex == 1 then
        self:BuildNurseryFeatures(folder, layerDef, palette)
        self:BuildWingForge(folder, layerDef, palette)
    elseif layerIndex == 2 then
        self:BuildMeadowFeatures(folder, layerDef, palette)
    elseif layerIndex == 3 then
        self:BuildCanopyFeatures(folder, layerDef, palette)
    elseif layerIndex == 4 then
        self:BuildStormwallFeatures(folder, layerDef, palette)
    elseif layerIndex == 5 then
        self:BuildLuminanceFeatures(folder, layerDef, palette)
    elseif layerIndex == 6 then
        self:BuildEmpyreanFeatures(folder, layerDef, palette)
    end

    -- 8. Community Board (shared stats display)
    self:CreateCommunityBoard(folder, layerDef, palette)

    -- 9. Angel Statue (landmark waypoint)
    self:CreateAngelStatue(folder, layerDef, palette, layerIndex)

    -- 10. Speed Boost Pads (fun traversal)
    self:CreateSpeedPads(folder, layerDef, palette, layerIndex)

    -- 11. NPC Gathering Area (social hub per layer)
    self:CreateGatheringArea(folder, layerDef, palette, layerIndex)

    -- 12. Hide brown starfish easter eggs (Claude/Anthropic tribute)
    self:HideStarfish(layerIndex, folder, layerDef)

    print("[WorldService] Layer " .. layerIndex .. " (" .. layerDef.name .. ") built: "
...
function WorldService:CreateSpawnPlatform(folder: Folder, position: Vector3, palette: any, layerIndex: number)
    ...
    self:AddBobAnimation(marker, 1.5)
...
function WorldService:CreateCloudPlatforms(folder: Folder, centerY: number, heightMin: number, heightMax: number, palette: any, layerIndex: number)
    ...
    for i = 1, platformCount do
        ...
        self:CreateCloudCluster(folder, pos, baseSize, palette, "CloudPlatform_" .. i)

        -- Some platforms get surface decorations
        if math.random() > 0.7 then
            self:AddSurfaceDecoration(nil, palette, folder, layerIndex, pos + Vector3.new(0, baseSize * 0.15, 0))
        end
    end

    -- Stepping stone paths between major areas
    self:CreateSteppingPaths(folder, heightMin, heightMax, palette, 3)
end

-- Build an organic cloud from overlapping spheres — looks like actual clouds
function WorldService:CreateCloudCluster(folder: Folder, center: Vector3, baseSize: number, palette: any, name: string)
...
function WorldService:AddSurfaceDecoration(platform: Part?, palette: any, folder: Folder, layerIndex: number, overridePos: Vector3?)
    local topPos
    if overridePos then
        topPos = overridePos
    ...
    if layerIndex == 1 then
        ...
        for j = 1, math.random(2, 4) do
            ...
            orb.Parent = folder
            self:AddBobAnimation(orb, 1 + math.random() * 2)
    ...
    elseif layerIndex == 6 then
        ...
        for j = 1, math.random(1, 3) do
            ...
            geo.Parent = folder
            self:AddBobAnimation(geo, 1 + math.random() * 2)
        end
    end
end

function WorldService:CreateSteppingPaths(folder: Folder, heightMin: number, heightMax: number, palette: any, pathCount: number)
    for p = 1, pathCount do
        ...
        for i = 1, stepCount do
            ...
            self:CreateCloudCluster(folder, stepPos, math.random(6, 12), palette, "Step_" .. p .. "_" .. i)
        end
    end
end
...
function WorldService:CreateFloatingIslands(folder: Folder, centerY: number, heightMin: number, heightMax: number, palette: any, layerIndex: number)
    ...
    for i = 1, islandCount do
        ...
        self:CreateCloudCluster(folder, pos, baseSize, palette, "FloatingIsland_" .. i)
        ...
        if math.random() > 0.6 then
            self:CreateCloudTree(folder, pos + Vector3.new(
...
function WorldService:CreateCloudTree(folder: Folder, basePos: Vector3, palette: any)
...
function WorldService:CreateDecorativeClouds(folder: Folder, heightMin: number, heightMax: number, palette: any)
    local cloudCount = 40
...
function WorldService:CreateLightPillars(folder: Folder, heightMin: number, heightMax: number, palette: any, layerIndex: number)
...
function WorldService:CreateReflectionPool(folder: Folder, layerDef: any, palette: any)
...
function WorldService:BuildNurseryFeatures(folder: Folder, layerDef: any, palette: any)
    local heightMin = layerDef.heightRange.min
    ...
    for i = 1, 6 do
        ...
        marker.Parent = folder
        self:AddBobAnimation(marker, 0.8)
    end
end

-- WING FORGE — glowing anvil station where you power up your wings
function WorldService:BuildWingForge(folder: Folder, layerDef: any, palette: any)
...
function WorldService:BuildMeadowFeatures(folder: Folder, layerDef: any, palette: any)
    local heightMin = layerDef.heightRange.min
    ...
    for i = 1, 4 do
        ...
        for j = 1, 5 do
            ...
            self:AddBobAnimation(particle, 5 + math.random() * 5)
        end
    end
    ...
    self:CreateCooperativeBridge(folder, layerDef, palette)

    -- Trial entrance portal
    self:CreateTrialPortal(folder, layerDef, palette)
end

function WorldService:CreateCooperativeBridge(folder: Folder, layerDef: any, palette: any)
...
function WorldService:CreateTrialPortal(folder: Folder, layerDef: any, palette: any)
...
function WorldService:BuildCanopyFeatures(folder: Folder, layerDef: any, palette: any)
    local heightMin = layerDef.heightRange.min
    local heightMax = layerDef.heightRange.max
    ...
    self:CreateBlessingBluff(folder, layerDef, palette)
...
function WorldService:BuildStormwallFeatures(folder: Folder, layerDef: any, palette: any)
    local heightMin = layerDef.heightRange.min
    local heightMax = layerDef.heightRange.max
...
function WorldService:BuildLuminanceFeatures(folder: Folder, layerDef: any, palette: any)
    local heightMin = layerDef.heightRange.min
    local heightMax = layerDef.heightRange.max
...
function WorldService:BuildEmpyreanFeatures(folder: Folder, layerDef: any, palette: any)
    local heightMin = layerDef.heightRange.min
    local heightMax = layerDef.heightRange.max
    local centerY = (heightMin + heightMax) / 2
    ...
    self:AddBobAnimation(altarOrb, 1)
...
function WorldService:CreateBlessingBluff(folder: Folder, layerDef: any, palette: any)
    local heightMin = layerDef.heightRange.min
    local bluffPos = Vector3.new(-50, heightMin + 30, -50)
...
function WorldService:CreateStarfish(parent: Folder, position: Vector3, scale: number?)
    scale = scale or 1
...
function WorldService:HideStarfish(layerIndex: number, folder: Folder, layerDef: any)
    ...
    if spots then
        ...
        for _, spot in ipairs(spots) do
            ...
            self:CreateStarfish(folder, pos, spot.scale)
            count = count + 1
        end
    else
        ...
        for i = 1, numStarfish do
            ...
            self:CreateStarfish(folder, pos, scale)
            count = count + 1
        end
    end

    print("[WorldService] Hidden " .. count .. " brown starfish in Layer " .. layerIndex
...
function WorldService:AddBobAnimation(part: BasePart, amplitude: number?)
...
function WorldService:AddDriftAnimation(part: BasePart)
    task.spawn(function()
...
function WorldService:CreateCommunityBoard(folder: Folder, layerDef: any, palette: any)
...
function WorldService:CreateAngelStatue(folder: Folder, layerDef: any, palette: any, layerIndex: number)
...
function WorldService:CreateSpeedPads(folder: Folder, layerDef: any, palette: any, layerIndex: number)
...
function WorldService:CreateGatheringArea(folder: Folder, layerDef: any, palette: any, layerIndex: number)
...
function WorldService:BuildLayerTransitions()
    for i = 1, 5 do
    ...
    print("[WorldService] Built 5 layer transition gates")
end
...
function WorldService:BuildGrandStairway()
    ...
    print("[WorldService] Built Grand Stairway: " .. totalSteps .. " cloud steps")
end
...
function WorldService:BuildWorldBoundaries()
    ...
    print("[WorldService] Built world boundaries (300 stud radius, cloud walls)")
end
