--[[
    BrainrotService.lua — Knit Service migration of BrainrotSystem
    Collectible brainrots scattered across layers
...
local BrainrotService = Knit.CreateService({
    ...
    Client = {
        BrainrotCollected = Knit.CreateSignal(),
        BrainrotDeposited = Knit.CreateSignal(),
        BrainrotUpdate = Knit.CreateSignal(),
    },
})
...
local brainrots: { any } = {}  -- { part, layerIndex, active, respawnTimer }

-- Player carry state (also stored as IntValue on character for devil system)
local playerCarrying: { [number]: number } = {}  -- [userId] = count
...
-- Reference to DataService (set during KnitStart)
local DataService: any = nil

function BrainrotService:KnitInit(): ()
    print("[BrainrotService] Initializing...")
end
...
function BrainrotService:KnitStart(): ()
    ...
    for layerIndex = 1, 6 do
        ...
        for i = 1, count do
            self:SpawnBrainrot(layerIndex, i)
        end
    end
    ...
    print("[BrainrotService] Spawned brainrots across all layers")
end

function BrainrotService:SpawnBrainrot(layerIndex: number, index: number): ()
...
function BrainrotService:Update(dt: number): ()
        ...
        if not br.active then
            ...
            if br.respawnTimer <= 0 then
                ...
                if br.part then
                    ...
                    local glowLight = br.part:FindFirstChildWhichIsA("PointLight")
                    if glowLight then glowLight.Enabled = true end
                    local emitterObj = br.part:FindFirstChildWhichIsA("ParticleEmitter")
                    if emitterObj then emitterObj.Enabled = true end
...
function BrainrotService:CollectBrainrot(player: Player, brainrotData: any): ()
    ...
    if brainrotData.part then
        ...
        local glowLight = brainrotData.part:FindFirstChildWhichIsA("PointLight")
        if glowLight then glowLight.Enabled = false end
        local emitterObj = brainrotData.part:FindFirstChildWhichIsA("ParticleEmitter")
        if emitterObj then emitterObj.Enabled = false end
    ...
    self.Client.BrainrotCollected:Fire(player, {
...
function BrainrotService:DepositBrainrots(player: Player): ()
    local userId = player.UserId
    ...
    -- Notify client via Knit signal
    self.Client.BrainrotDeposited:Fire(player, {
        deposited = carrying,
...
function BrainrotService:GetCarrying(player: Player): number
    return playerCarrying[player.UserId] or 0
end

function BrainrotService:OnPlayerJoined(player: Player): ()
    playerCarrying[player.UserId] = 0
end

function BrainrotService:RemovePlayer(player: Player): ()
