--[[
    RetroService.lua — Retro easter eggs scattered throughout The Cloud Climb (Knit)
...
local RetroService = Knit.CreateService({
    Name = "RetroService",
    Client = {
        DialCode = Knit.CreateSignal(),
        DialResult = Knit.CreateSignal(),
        SignalAppear = Knit.CreateSignal(),
    },
})

-- Valid dial codes and their rewards
-- Players discover codes from lore fragments, The Signal hints, or community sharing
local DIAL_CODES: { [string]: { reward: number, message: string, oneTime: boolean?, cooldown: number? } } = {
...
local playerDialState: { [number]: { redeemedCodes: { [string]: boolean }, cooldowns: { [string]: number } } } = {}

-- References to other Knit services (resolved in KnitStart)
local DataService
local MoteService

function RetroService:KnitInit(): ()
    -- Listen for client dial code submissions via Knit signal
    self.Client.DialCode:Connect(function(player: Player, code: string)
        self:HandleDial(player, code)
    end)
    ...
    task.spawn(function()
        while true do
            ...
            self:SpawnSignalEvent()
        end
    end)

    print("[RetroService] Retro system initialized — dial in, tune in, climb on")
end

function RetroService:KnitStart(): ()
...
function RetroService:CreatePhoneBooth(parent: Folder, position: Vector3, rotation: number?): (Model, ProximityPrompt)
...
function RetroService:HandleDial(player: Player, code: string): ()
    ...
    if not codeData then
        ...
        self.Client.DialResult:Fire(player, {
            success = false,
    ...
    MoteService:AwardMotes(player, codeData.reward, "rotary_phone")
    ...
    print("[RetroService] " .. player.Name .. " dialed " .. code .. " -> +" .. codeData.reward .. " Motes")
end
...
function RetroService:CreateSignalNPC(parent: Instance, position: Vector3): (Model, ProximityPrompt)
    ...
    task.spawn(function()
        local glitchTexts = { "SIGNAL", "S1GN4L", "S\204\184I\204\184G\204\184N\204\184A\204\184L", "516N4L", "-----", "HELLO", "TUNE IN", "???.???" }
...
function RetroService:SpawnSignalEvent(): ()
    ...
    local signalModel, prompt = self:CreateSignalNPC(workspace, spawnPos)

    -- Wire prompt
    prompt.Triggered:Connect(function(player: Player)
        self:OnSignalInteract(player)
    end)

    -- Notify nearby players
    self.Client.SignalAppear:FireAll({
        position = spawnPos,
...
function RetroService:OnSignalInteract(player: Player): ()
    -- Pick a random glitchy line
    local line = SIGNAL_LINES[math.random(#SIGNAL_LINES)]

    -- Send as dialogue via NPCService's client signal
    local NPCService = Knit.GetService("NPCService")
    NPCService.Client.NPCDialogue:Fire(player, {
        npcId = "the_signal",
        npcName = "The Signal",
        lines = {
    ...
    MoteService:AwardMotes(player, 1, "signal_encounter")
...
function RetroService:CreateBoombox(parent: Folder, position: Vector3): Model
...
function RetroService:CreateArcadeCabinet(parent: Folder, position: Vector3): Model
...
function RetroService:PopulateLayer(layerFolder: Folder, layerIndex: number, layerDef: any): ()
    ...
    local _, boothPrompt = self:CreatePhoneBooth(layerFolder, boothPos)
    boothPrompt.Triggered:Connect(function(player: Player)
    ...
    for i = 1, math.random(1, 2) do
        self:CreateBoombox(layerFolder, Vector3.new(
            math.random(-150, 150),
    ...
    if layerIndex >= 2 then
        self:CreateArcadeCabinet(layerFolder, Vector3.new(
            math.random(-120, 120),
...
function RetroService:RemovePlayer(player: Player): ()
