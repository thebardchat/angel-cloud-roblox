--[[
    CloudBaseService.lua — Personal cloud base for each player (Knit Service)
    ...
    Migrated from ServerScriptService/CloudBaseSystem.lua to Knit service pattern.
...
local CloudBaseService = Knit.CreateService({
    Name = "CloudBaseService",
    Client = {
        BaseUpdated = Knit.CreateSignal(),
    },
})
...
-- Active bases
local playerBases: { [number]: { model: Model, depositPad: BasePart, level: number, position: Vector3 } } = {}
...
function CloudBaseService:KnitInit(): ()
    print("[CloudBaseService] Initializing cloud base service")
end

function CloudBaseService:KnitStart(): ()
    DataService = Knit.GetService("DataService")

    -- BrainrotService may or may not exist yet; resolve safely
    local ok, svc = pcall(function()
        return Knit.GetService("BrainrotService")
    end)
    if ok then
        BrainrotService = svc
    end
    ...
    print("[CloudBaseService] Cloud base system initialized")
end

--[[ ── Player Lifecycle ─────────────────────────────────────────────── ]]

function CloudBaseService:OnPlayerJoined(player: Player): ()
    ...
    self:BuildBase(player, basePos, baseLevel)
end

function CloudBaseService:RemovePlayer(player: Player): ()
...
function CloudBaseService:BuildBase(player: Player, position: Vector3, level: number): ()
    local userId = player.UserId
    ...
    -- Notify clients about the base update
    self.Client.BaseUpdated:FireAll(player.UserId, level, position)

    print("[CloudBaseService] Built base for " .. player.Name .. " (Level " .. level .. ")")
end

--[[ ── Deposit Check ────────────────────────────────────────────────── ]]

function CloudBaseService:CheckDeposit(player: Player): ()
    local userId = player.UserId
    local base = playerBases[userId]
    ...
    if distance < DEPOSIT_RADIUS then
        ...
        if not BrainrotService then return end

        local carrying = BrainrotService:GetCarrying(player)
        if carrying > 0 then
            BrainrotService:DepositBrainrots(player)
            ...
            if data then
                ...
                if newLevel > base.level then
                    ...
                    self:BuildBase(player, base.position, newLevel)
                end
            end
        end
    end
...
function CloudBaseService:GetBasePosition(player: Player): Vector3?
...
function CloudBaseService:IsInSafeZone(position: Vector3): boolean
...
function CloudBaseService.Client:GetBasePosition(player: Player): Vector3?
    return self.Server:GetBasePosition(player)
end

function CloudBaseService.Client:IsInSafeZone(player: Player, position: Vector3): boolean
    return self.Server:IsInSafeZone(position)
end
