--[[
    AtmosphereService.lua — Per-layer atmosphere, lighting, and weather (Knit)
...
local AtmosphereService = Knit.CreateService({
    ...
    Client = {
        AtmosphereUpdate = Knit.CreateSignal(),
    },
})
...
local playerLayers: { [number]: number } = {}

-- Reference to SoundManagerService (resolved in KnitStart)
...
function AtmosphereService:KnitInit(): ()
    ...
    print("[AtmosphereService] Atmosphere system initialized")
end
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            ...
            if character then
                ...
                if hrp then
                    ...
                    local newLayer = self:GetLayerForHeight(height)
                    ...
                    if newLayer ~= currentLayer then
                        ...
                        if preset then
                            self.Client.AtmosphereUpdate:Fire(player, {
                                layerIndex = newLayer,
                                preset = {
                        ...
                        SoundManagerService:SetAmbientLayer(newLayer)
                        SoundManagerService:OnPlayerLayerChanged(player, newLayer)
                    end
                end
            end
        end
...
function AtmosphereService:GetLayerForHeight(height: number): number
...
function AtmosphereService:ApplyPreset(layerIndex: number): ()
...
function AtmosphereService:TriggerLightningFlash(): ()
    ...
    -- Play thunder sound
    SoundManagerService:OnLightning()
...
function AtmosphereService:RemovePlayer(player: Player): ()
    playerLayers[player.UserId] = nil
end

function AtmosphereService:GetPlayerLayer(player: Player): number
